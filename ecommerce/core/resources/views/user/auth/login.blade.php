  ...
  <div class="row">
          ...
          <div class="col-md-6">
            ...
            <form class="row" action="{{route('user.register.submit')}}" method="POST">
              ...
              <div class="col-12 text-center" id="otp-b">
                <a class="btn btn-primary btn-sm" id="register-btn">{{__('Register')}}</a>
              </div>

              <div class="col-sm-6" id="otp-f">
                <div class="form-group">
                  <label for="reg-pass-confirm">OTP</label>
                  <input class="form-control" type="number" name="otp" placeholder="Enter OTP" id="otp-e">
                </div>
              </div>
              <input type="hidden" name="rq">

              {{-- <div class="col-12 text-center" id="otp-s">
                <a class="btn btn-primary btn-sm" id="register-btn">{{__('Register')}}</a>
              </div> --}}

              <div class="col-12 text-center">
                <button class="btn btn-primary margin-bottom-none btn-sm" id="sub-full" type="submit">{{__('Register')}}</button>
              </div>
            </form>
                </div>
            </div>
...
@section('script')
    <script>
        ...
        $("#register-btn").click(function (e) {
            ...
            $.ajax({
                type:'POST',
                url:"{{ route('user.register.otp') }}",
                data:{"otp_phone":phone, "_token": "{{ csrf_token() }}", },
